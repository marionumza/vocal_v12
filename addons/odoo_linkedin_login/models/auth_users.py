...
class ResUsers(models.Model):
	...
	def _generate_signup_values(self, provider, validation, params):
		if self.env['auth.oauth.provider'].browse(provider).name.lower() == 'linkedin':
			oauth_uid = validation['user_id']
			email = validation.get('email', 'provider_%s_user_%s' % (provider, oauth_uid))
			name = validation.get('localizedFirstName', email)
		else:
			oauth_uid = validation['user_id']
			email = validation.get('email', 'provider_%s_user_%s' % (provider, oauth_uid))
	...
	def auth_oauth(self, provider, params):
		...
		if self.env['auth.oauth.provider'].browse(provider).name.lower() == 'linkedin':
			...
			linkedin_client_id = self.env['auth.oauth.provider'].browse(provider).client_id
			linkedin_client_secret = self.env['auth.oauth.provider'].browse(provider).client_secret
			redirect_url = Parameters.get_param('web.base.url')
			headers = {"Content-type": "application/x-www-form-urlencoded"}
			data = {
			  'grant_type': 'authorization_code',
			  'redirect_uri': str(redirect_url)+'/auth_oauth/signin',
			  'client_id': linkedin_client_id,
			  'client_secret': linkedin_client_secret,
			  'code': params['code']
			}

			response = requests.request("POST", LINKEDIN_TOKEN_ENDPOINT, data=data, headers=headers)
			response_data = json.loads(response.text)
			params['access_token'] = response_data['access_token']
