...
class UPSAPI:
    ...
    def get_tracking_link(cls,tracknum):
        ups_tracking_url = "http://wwwapps.ups.com/etracking/tracking.cgi"
        return '%s?tracknum=%s'%(ups_tracking_url,quote_plus(tracknum))
    ...
    def construct_address(self,addr_type,data):
        ...
        self.add_text(SubElement(AddressRoot, 'Name'),data.get('name'))
        self.add_text(SubElement(AddressRoot, 'PhoneNumber'),data.get('phone'))
        self.add_text(SubElement(AddressRoot, 'CompanyName'), data.get('company_name') or data.get('name'))
        self.add_text(SubElement(AddressRoot, 'AttentionName'),data.get('name'))
        ...
        self.add_text(SubElement(Address, 'AddressLine1'),data.get('street'))
        self.add_text(SubElement(Address, 'AddressLine2'),data.get('street2'))
    ...
    def contruct_void_req(self,data):
        ...
        void_shipment = SubElement(root_node, 'ExpandedVoidShipment')
...
class UPSDeliveryCarrier(models.Model):
    ...
    def ups_get_tracking_link(self, picking):
        return UPSAPI.get_tracking_link(picking.ups_shipment_number)
