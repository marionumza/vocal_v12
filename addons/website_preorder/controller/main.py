...
class WebsiteSale(WebsiteSale):
    ...
    def get_order_id(self, product_id, add_qty=None, set_qty=0, **kw):
        ...
        add_percent_msg = active_config.get('add_pre_order_msg') if active_config.get(
            'add_pre_order_msg') else "You cannot place an order for a partial payment pre order product with other products, please empty your cart first to place your order for a partial payment pre order product"
        stock_value = request.website.get_preorder_product_stock_qty(product)
        ...
        if res and res.order_line:
            if res.preorder_payment_state == 'partial':
                return {'status': False,
                        'msg': _("Please Complete you current Order before proceeding to Further orders.")}
            if any(
                    line.is_preorder and line.product_id.payment_type == 'percent' and line.product_id.id != product.id and request.website.get_preorder_product_stock_qty(
                            line.product_id) <= 0 for line in res.order_line):
                    ...
                    if request.env['website'].get_pre_order(product):
                        if product.payment_type == 'percent':
                            if stock_value <= 0 and total_qty > product.max_order_qty:
                                return {'status': False,
                                        'msg': _("You are Trying to add more than available quantity of the Product.")}
                            elif stock_value > 0 and total_qty > stock_value:
                                if int(line.product_uom_qty) < stock_value:
                                    return {'status': False, 'msg': _(
                                        "You are Trying to add more than available quantity of the Product.")}
                                else:
                                    return {'status': False, 'msg': _(add_percent_msg)}
    ...
    def cart_update(self, product_id, add_qty=1, set_qty=0, **kw):
        ...
        if product_id and sale_order_id:
            ...
            if sale_order and sale_order.sudo().order_line:
                ...
                if any(
                        line.is_preorder and line.product_id.payment_type == 'percent' and line.product_id.id != product.id and request.website.get_preorder_product_stock_qty(
                                line.product_id) <= 0 for line in sale_order.sudo().order_line):
                    return False
    ...
    def payment_validate(self, transaction_id=None, sale_order_id=None, **post):
        ...
        if sale_order_id:
            ...
            if order.is_preorder:
                ...
                if not any(line.is_preorder and line.product_id.payment_type == 'percent' for line in
